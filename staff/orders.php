...
$order = mysqli_query($conn,"SELECT * FROM `orders` where status='1'");
?>
...
<td>Pending</td>
<td><a href="functions/order/move.php?id=<?php echo $row['id']; ?>&action=s" class="btn btn-warning">Move To Shipped</a><br><br><a href="functions/order/view.php?oid=<?php echo $row['order_no']; ?>" class="btn btn-primary">View Details</a><br><br><a href="functions/order/move.php?id=<?php echo $row['id']; ?>&action=r" class="btn btn-danger">Reject Order</a></td>
</tr>
                                    <?php } ?>
                                 </tbody>
                              </table>
