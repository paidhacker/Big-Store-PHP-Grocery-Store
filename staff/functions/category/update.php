...
while($row = mysqli_fetch_assoc($cat)){
                        ?>
                           <div class="form-group"><label for="company" class=" form-control-label">Name</label><input type="text" name="name" id="company" placeholder="Enter your category Name (Max: 20)" value="<?php echo $row['name']; ?>" maxlength="20" class="form-control" required></div>
                           <div class="form-group"><label for="company" class=" form-control-label">Image (1600 X 450)&nbsp;</label><input type="file" name="img1" id="file" value="null"></div>
<?php } ?>
         ...
         if($file_error === 0){
            ...
            if(move_uploaded_file($file_tmp, $file_destination)){
               if($file_size >= 1){
                  ...
                  mysqli_query($conn, "UPDATE `categories` SET `img` = '$rimg' WHERE id = '$id'");
